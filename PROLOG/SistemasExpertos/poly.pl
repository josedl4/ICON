%%%% Sistema Experto par la comprobación basica de distintos poligonos. %%%%

% Reglas para los triangulos %
...
% Implementación de las preguntas posibles a realizar sobre los poligonos %
:-dynamic memory/2.
...
ladosIguales(X):-ask(ladosIguales, '¿Cuantos lados 
    iguales tiene la figura? ', X).
anguloRecto(X):-ask(anguloRecto, '¿La figura posee 
    angulos rectos (si, no)? ', X).
ladosParalelos(X):-ask(ladosParalelos, '¿Cuantos lados 
    paralelos tiene la figura (0, 2 o 4)? ', X).
